...
public class ClienteDAO {
    ...
    public List<Cliente> listarTodos() {
        // 3. Usa JPQL (Java Persistence Query Language) para buscar os dados. Observe que usamos "Cliente" (nome da classe) e não o nome da tabela no SQL.
    ...
    public void apagar(long id){
        try{            
            // 4. No JPA, não é possível remover um objeto diretamente passando apenas uma instância nova com o ID. O ciclo de vida do JPA exige que o objeto seja colocado 
            //    no estado Gerenciado (Managed) primeiro. Por isso, precisamos do em.find() e, caso exista, chama-se o em.remove().

            // 5. Como o relacionamento mapeado na classe Cliente possui a configuração de cascata, ao apagar um cliente, todos os telefones vinculados a ele serão automaticamente 
    ...
    public void editar(Cliente cliente){
        try{            
            // 6. Quando passamos um objeto Cliente que já possui um id preenchido, o Hibernate detecta que ele já existe no 
            // banco de dados e executa um comando SQL UPDATE em vez de um INSERT.
            em.getTransaction().begin();
            em.merge(cliente);
            em.getTransaction().commit();
